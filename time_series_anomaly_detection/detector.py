...
class GAN_AD(TimeSeriesAnomalyDetector):
    ...
    def __init__(
            ...
            batch_size: int = 8,
            latent_dim: int = 1,
            discriminator_params: Dict = {'lstm_units': 150,
                                          'dense_units': 50,
                                          'dropout_rate': 0.2},
            generator_params: Dict = {'conv_filters': 30,
                                      'conv_kernel_size': 3,
                                      'lstm_units': 200},
    ...
    def _build_gan_ad(self) -> keras.Model:
        ...
        model = keras.Model(inputs=[z], outputs=[generated_series, generated_series_eval],
                            name='gan')
        return model
        ...
        for epoch in range(epochs):
            ...
            if (epoch != 0) and (epoch % print_frequency == 0) and enable_prints:
                ...
                for i in range(9):
                    ...
                    generated_sample = self._generator(tf.reshape(z_init,
                                                                  [1, self._window_size, self._latent_dim]),
                                                       training=False)[0]
    ...
    def _get_dataset(self, data: pd.DataFrame) -> tf.data.Dataset:
        ...
        dataset = tf.keras.preprocessing.timeseries_dataset_from_array(
            data_scaled.astype(np.float), targets=None, sequence_length=self._window_size,
            sequence_stride=self._shift, sampling_rate=1, batch_size=self._batch_size,
            shuffle=True, seed=6)
        return dataset
    ...
    def fit(self, X: pd.DataFrame, n_epochs: int = 10, d_learning_rate: float = 0.0002,
        ...
        # train GAN-AD model
        self._train(dataset, n_epochs, save_checkpoints, saving_frequency,
                    enable_prints, print_frequency)
        pass
    ...
    def _find_mapping(self, sample: pd.DataFrame, max_iterations: int = 1000, tolerance: float = 0.05) -> tf.Tensor:
        ...
        def get_loss():
            generated_sample = self._generator(tf.reshape(z_opt,
                                                          [1, self._window_size, self._latent_dim]), training=False)[0]
    ...
    def predict_window_anomaly_scores(self, X: pd.DataFrame, max_iterations: int = 1000,
        ...
        generated_sample = self._generator(tf.reshape(z_mapping,
                                                      [1, self._window_size, self._latent_dim]), training=False)[0]
        ...
        real_prob = self._discriminator(tf.reshape(generated_sample,
                                                   [1, generated_sample.shape[0], generated_sample.shape[1]]),
                                        training=False)[0]
        ...
        # is not weighted output from the discriminator, 
        # but weighted (1 - output from the discriminator) instead 
