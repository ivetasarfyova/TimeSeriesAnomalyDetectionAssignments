...
class GAN_AD(TimeSeriesAnomalyDetector):
        ...
        for epoch in range(epochs):
            ...
            if ((epoch != 0) and (epoch % 500 == 0) and enable_prints):
                ...
                for i in range(9):
                    ...
                    generated_sample = \
                    self._generator(tf.reshape(z_init, [1, self._window_size, self._latent_dim]), training=False)[0]
    ...
    def _find_mapping(self, sample: tf.Tensor) -> tf.Tensor:
        ...
        def get_loss():
            generated_sample = \
            self._generator(tf.reshape(z_opt, [1, self._window_size, self._latent_dim]), training=False)[0]
    ...
    def predict_window_anomaly_scores(
        ...
        generated_sample = \
        self._generator(tf.reshape(z_mapping, [1, self._window_size, self._latent_dim]), training=False)[0]
        print('sample')
        ...
        real_prob = \
        self._discriminator(tf.reshape(generated_sample, [1, generated_sample.shape[0], generated_sample.shape[1]]),
                            training=False)[0]
        print('real_prob')
        print(real_prob)
    ...
    def predict_anomaly_scores(
        ...
        if (self._id_columns == None):
            ...
            [df.drop(columns=self._id_columns, inplace=True) for df in dfs]
            anomaly_score = pd.concat([self.predict_series_anomaly_scores(df) for df in dfs], axis=0).reset_index(
                drop=True)
        return anomaly_score
