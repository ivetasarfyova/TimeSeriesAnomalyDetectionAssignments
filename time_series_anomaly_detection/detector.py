...
class GAN_AD(TimeSeriesAnomalyDetector):
    ...
    def _get_discriminator_loss(self, real, generated):
    ...
    def _get_generator_loss(self, generated):
        return self._bce(tf.ones_like(generated), generated)
    
    @tf.function
    def _fit_batch(self, batch, z):
    ...
    def _train(self, dataset, epochs, enable_prints=False):
        loss_history = []
        for epoch in range(epochs):
            ...
            if ((epoch != 0) & (epoch%500 == 0) & enable_prints):
                print(epoch, ". epoch")
    ...
    def fit(self, X: pd.DataFrame, n_epochs, d_learning_rate=0.0002,
            g_learning_rate=0.00002, save_checkpoints=False, enable_prints=False, *args, **kwargs) -> None:
        #prepocesing dat
    ...
    def _get_mmd_similarity(self, x, y):
        xy_kernel = self._rbf_kernel(x, y)
        return xy_kernel
    
    def _find_mapping(self, sample):
        tolerance = 0.05
    ...
    def predict_anomaly_scores(
        ...
        real_prob = self._discriminator(tf.reshape(generated_sample, [1, generated_sample.shape[0], generated_sample.shape[1]]), training = False)[0]
        print('real_prob')
        print(real_prob)
    ...
    def identify_anomaly(self, X, treshold = 0.9):
